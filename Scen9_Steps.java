/*
Feature 4: Invite a friend

As a user
I want to invite a friend
So that he can join the site 

Scenario: Unsuccessful invite (already invited)

Given the user is logged in
When user clicks ‘Invite a friend’ link
And types ‘kudrix@bk.ru’ in ‘To’ text window
And clicks ‘Send invite in email’ button
Then the system should display message: ‘The following recipient is already a member’
...
public class Scen9_Steps {
	  ...
	  public void the_system_should_display_message_The_following_recipient_is_already_a_member() throws Throwable {
		    assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*The following recipient is already a member[\\s\\S]*$"));
