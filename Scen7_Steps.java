...
Scenario: Successful Pin search (match all that include)

Given PinRn.com is loaded 
When the user types ‘health’ in the search window
And clicks Enter
Then the system should display all posts that contain ‘Healthy’ in their title
*/
...
public class Scen7_Steps {
	  ...
	  public void the_user_types_An_Introduction_to_Green_Parenting_in_the_search_window() throws Throwable {
		    ...
		    driver.findElement(By.id("edit-body-value")).sendKeys("health");
	  }
	  ...
	  @Then("^the system should display all posts that contain ‘Healthy’ in their title$")
	  public void the_system_should_display_all_posts_that_contain_Healthy_in_their_title() throws Throwable {
		    assertTrue(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Healthy[\\s\\S]*$"));
